...
description: LunaTranslator Tutorial — An open-source GalGame / visual novel translator that supports HOOK, OCR, and AI translation engines. Integrates with __WEBSITE_NAME__ to improve translation quality.
---

<Callout type="info" title="Chat Settings Options">
  In the __WEBSITE_NAME__
...
LunaTranslator supports integration with locally deployed __WEBSITE_NAME__. Please use your own deployed __WEBSITE_NAME__, or ensure that the service provider has legitimate upstream authorization and compliance obligations for their __WEBSITE_NAME__ service. Do not connect API addresses or keys from unknown sources to production environments.

### One-Click Configuration

1.  In the __WEBSITE_NAME__ console's `System Settings` -> `Chat Settings`, add the following shortcut option:
