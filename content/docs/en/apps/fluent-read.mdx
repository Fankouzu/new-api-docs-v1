...
description: FluentRead configuration guide — open-source browser translation plugin with 20+ engines and AI model support. Connect to ((WEBSITE_NAME)) for immersive bilingual reading.
---

import { WebsiteName } from '@/components/website-name';

<Callout type="info" title="Chat Settings Options">
  In the System Settings -> Chat Settings of the <WebsiteName /> console, you can add
...
### Import Configuration from <WebsiteName /> Console (Recommended)

After the FluentRead plugin is installed in your browser, opening the <WebsiteName /> console -> Token Management page will display a prompt to add FluentRead.
...
After confirming the import, the <WebsiteName /> configuration in FluentRead will be enabled.
...
| ------------------- | --------------------------------------- |
| Translation Service | <WebsiteName />                                  |
| Access Token        | <WebsiteName /> Key                              |
| <WebsiteName /> Endpoint     | <WebsiteName /> Deployment Address (without /v1) |
