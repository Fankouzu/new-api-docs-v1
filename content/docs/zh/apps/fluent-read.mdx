...
description: FluentRead 教程 — 开源浏览器翻译插件，支持 20+ 翻译引擎与 AI 大模型翻译。对接 ((WEBSITE_NAME)) 实现沉浸式双语阅读体验。
---
...
### 从 <WebsiteName /> 控制台导入配置（推荐）

当浏览器安装了流畅阅读插件后，打开 <WebsiteName /> 控制台->令牌管理页面会弹出添加流畅阅读的提示
...
确认导入后在流畅阅读中的<WebsiteName />配置便会启用
...
| 翻译服务   | <WebsiteName />                     |
| 访问令牌   | <WebsiteName /> 密钥                |
| <WebsiteName />接口 | <WebsiteName />部署地址（不带/v1）  |
| 模型       | 列表中选择，或者自定义模型 |
| 自定义模型 | 模型名称                   |
